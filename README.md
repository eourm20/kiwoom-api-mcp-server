...
### Quant Trading 도구 (quant_trading 브랜치)

| 도구 | 설명 |
|---|---|
| `quant_report` | 현황 조회 (`type`: signals/portfolio/trades/strategy/all, `days`, `limit`) |
| `quant_strategy_log` | 전략 결정 기록 및 텔레그램 발송 (`category`: trade/watchlist/general) |
| `quant_portfolio_sync` | 포트폴리오 및 매매내역 DB 동기화 |


## 환경 변수 설정

[`kiwoom_mcp/.env.example`](kiwoom_mcp/.env.example) 참고

| 변수 | 설정 위치 | 설명 |
|---|---|---|
| `KIWOOM_APP_KEY` | `.env` | 키움 OpenAPI 앱 키 |
| `KIWOOM_APP_SECRET` | `.env` | 앱 시크릿 키 |
| `KIWOOM_ACCOUNT_NO` | `.env` | 조회/주문 대상 계좌번호 |
| `KIWOOM_BASE_URL` | `claude_desktop_config.json` | 실전/모의투자 구분 URL |
| `KIWOOM_ALLOW_TRADE_EXECUTION` | `claude_desktop_config.json` | `true`일 때만 주문/정정/취소 허용 (기본: `false`) |
| `KIWOOM_CATALOG_PATH` | `.env` (수정 불필요) | API 카탈로그 문서 경로 |
| `KIWOOM_API_PDF_PATH` | `.env` (수정 불필요) | 키움 API PDF 문서 경로 |
| `QUANT_TRADING_PATH` | `claude_desktop_config.json` | quant_trading 프로젝트 루트 경로 (quant_trading 브랜치) |

**우선순위:** `claude_desktop_config.json` env > `kiwoom_mcp/.env` > 프로젝트 루트 `.env`
...
`kiwoom_mcp\.env`를 열어 아래 **3개의 개인 인증 정보**만 본인 값으로 변경합니다.
...
나머지 설정(`KIWOOM_BASE_URL`, `KIWOOM_ALLOW_TRADE_EXECUTION`)은 아래 Claude Desktop 설정에서 지정합니다.
...
- `KIWOOM_BASE_URL`: 실전 `https://api.kiwoom.com` / 모의투자 `https://mockapi.kiwoom.com`
- `KIWOOM_ALLOW_TRADE_EXECUTION`: 주문/정정/취소 허용 여부 (`false` 기본, 실거래 시 `true`)
- `QUANT_TRADING_PATH`: quant_trading 프로젝트 루트 경로 (quant_trading 브랜치 사용 시)
...
```json
{
  "mcpServers": {
    "kiwoom-mcp": {
      "command": "C:\\Users\\yourname\\kiwoom-api-mcp-server\\.venv\\Scripts\\python.exe",
      "args": ["-m", "kiwoom_mcp.server"],
      "env": {
        "PYTHONPATH": "C:\\Users\\yourname\\kiwoom-api-mcp-server",
        "MCP_TRANSPORT": "stdio",
        "KIWOOM_BASE_URL": "https://api.kiwoom.com",
        "KIWOOM_ALLOW_TRADE_EXECUTION": "false",
        "QUANT_TRADING_PATH": "C:\\Users\\yourname\\quant_trading"
      }
    }
  }
}
